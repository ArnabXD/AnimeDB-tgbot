import ava from 'ava';
import getInlineResults from './inlineResults';

ava('inline result (anime)', async (test) => {
...
ava('inline result (manga)', async (test) => {
  test.deepEqual(await getInlineResults('Tokyo Revengers', 'MANGA'), {
    Page: {
      pageInfo: {
        total: 2,
        currentPage: 1,
