import { Media } from "./anilist/getMedia.ts";
...
export const parseMedia = (
  ...
  if (synopsis) {
    synopsis = synopsis.length > 900 - message.length
      ? synopsis.substring(0, 900 - message.length) +
        `... <a href="https://anilist.co/${type.toLowerCase()}/${media.id}">Read More</a>`
      : synopsis;
